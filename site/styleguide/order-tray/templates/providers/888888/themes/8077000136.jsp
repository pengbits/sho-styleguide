...
<figure class="order-card{{#selected}} order-card--selected{{/selected}}{{#isLast}} order-card--last{{/isLast}}{{#isFirst}} order-card--first{{/isFirst}}{{#isTrueFirst}} order-card--true-first{{/isTrueFirst}}" data-provider-id="{{id}}" data-context="provider card" data-position="{{position}}">
  <figcaption class="order-card__inner">
    <div {{#is_mobile}}class="order-card__toggle" {{^selected}}data-track data-label="open card"{{/selected}}{{/is_mobile}}>
      <img class="order-card__logo" src="http://www.sho.com/assets/images/order/tray/provider-logos/smart-tvs.png">
    </div>
    
    <span class="order-card__free-trial-callout">7-Day Free Trial&#42;</span>
    
    <h4 class="order-card__headline">Download the SHOWTIME app on your Samsung Smart TV</h4>
    {{> partials/order/templates/partnerLogosSmartTVs }}
    <div class="order-card__body">

      <div class="order-card__description">
        Go to the Samsung App Store on your supported Samsung Smart TV. Download and open the SHOWTIME app, then follow the on-screen instructions to sign up and START YOUR FREE TRIAL. Cancel any time.
        <br />&nbsp;
      </div>
    </div>
    ...
    <div class="order-card__details {{#expanded}}order-card__details--expanded{{/expanded}}">
      ...
      <div class="order-card__blurb order-card__blurb--devices">
        ...
        <h5>Choose how you want to watch! Once you sign-up through the SHOWTIME app on your Samsung Smart TV you can download the app on other connected TVs, tablets and mobile devices &#8211; or go to Showtime.com on your computer &#8211; and stream SHOWTIME at NO ADDITIONAL COST.</h5>
