...
class StreamingModal
{
  constructor(){
   ...
   this.heroImageClassName = 'streaming-modal--hero-image';
   this.configDefault = {
     'show_logos':false,
  ...
  setListeners() {
    ...
    Variations.on('variation:detected:sho', (event, data) => {
      ...
      if(!!data && !!data.streaming_modal){
        ...
        if(!this.config.hero_image){
          this.heroImageClassName = '';
        } else {
          if(this.config.episode_number){
            this.config.hero_image_title = this.config.show_title + " " + this.config.episode_number + " - " + this.config.episode_title;
          }else{
            // replace 'EPISODE' with 'MOVIE' for movie page
            this.config.hero_image_headline = this.config.hero_image_headline.toLowerCase().replace("episode", "movie").toUpperCase();
            this.config.hero_image_title = this.config.episode_title;
          }
        }
  ...
  getTemplate() {
   return `<div class="streaming-modal modal-open ${this.hideLogosClassName} ${this.redOrderPromoClassName} ${this.heroImageClassName}" data-context="streaming modal">
    <div class="streaming-modal__hero-image">
    <h2 class="streaming-modal__hero-image-headline">${this.config.hero_image_headline}</h2>
    <p class="streaming-modal__copy">${this.config.hero_image_title}</p>
    <div class="streaming-modal__hero-image-container">
    <img src="${this.config.hero_image_src}" alt="${this.config.hero_image_title}">
    </div>
    </div>
    ...
    <a class="streaming-modal__link" href="${this.showtimeLink}${this.newSlug}" data-track data-label="standalone">${this.config.direct_label}</a>
    ...
    <a class="streaming-modal__link" href="${this.showtimeAnytimeLink}${this.newSlug}" data-track data-label="anytime">${this.config.anytime_label}</a>
